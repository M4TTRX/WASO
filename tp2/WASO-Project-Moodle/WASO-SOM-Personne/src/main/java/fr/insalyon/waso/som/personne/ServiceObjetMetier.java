...
public class ServiceObjetMetier {
    ...
    public void getListePersonne() throws ServiceException {
        try {
            List<Object[]> listePersonne = this.dBConnection
                    .launchQuery("SELECT PersonneID, Nom, Prenom, Mail FROM PERSONNE ORDER BY PersonneID");
    ...
    public void getPersonneParId(Integer personneId) throws ServiceException {
        try {
            List<Object[]> listePersonne = this.dBConnection.launchQuery(
                    "SELECT PersonneID, Nom, Prenom, Mail FROM PERSONNE WHERE PersonneID = ? ORDER BY PersonneID",
                    personneId);
    ...
    public void rechercherPersonneParNom(String nomPersonne) throws ServiceException {
        try {
            String queryParam = '%' + nomPersonne + '%';
            List<Object[]> listePersonne = this.dBConnection.launchQuery(
                    "SELECT PersonneID, Nom, Prenom, Mail FROM PERSONNE WHERE Nom LIKE UPPER(?) ORDER BY PersonneID",
                    queryParam);
